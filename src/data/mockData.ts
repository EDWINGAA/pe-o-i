import { User, Barbershop, Barber, Service, Appointment } from '../types';

// Usuarios simulados
export const mockUsers: User[] = [
  {
    id: '1',
    email: 'cliente@test.com',
    password: '123456',
    name: 'Juan Pérez',
    role: 'client',
    phone: '555-0101'
  },
  {
    id: '2',
    email: 'barbero@test.com',
    password: '123456',
    name: 'Carlos Martínez',
    role: 'barber',
    phone: '555-0202',
    barberId: '1'
  },
  {
    id: '3',
    email: 'barbero2@test.com',
    password: '123456',
    name: 'Miguel Ángel',
    role: 'barber',
    phone: '555-0203',
    barberId: '2'
  }
];

// Barberías simuladas
export const mockBarbershops: Barbershop[] = [
  {
    id: '1',
    name: 'Barbería El Clásico',
    address: 'Calle Principal #123, Centro',
    phone: '555-1000',
    rating: 4.8,
    image: 'https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=400',
    description: 'La mejor barbería tradicional de la ciudad con más de 20 años de experiencia',
    openTime: '09:00',
    closeTime: '20:00',
    latitude: 19.432608,
    longitude: -99.133209
  },
  {
    id: '2',
    name: 'Style & Cuts',
    address: 'Av. Insurgentes #456, Col. Roma',
    phone: '555-2000',
    rating: 4.6,
    image: 'https://images.unsplash.com/photo-1585747860715-2ba37e788b70?w=400',
    description: 'Barbería moderna con los mejores estilos urbanos',
    openTime: '10:00',
    closeTime: '21:00',
    latitude: 19.421608,
    longitude: -99.163209
  },
  {
    id: '3',
    name: 'Barbershop Premium',
    address: 'Blvd. Miguel de Cervantes #789',
    phone: '555-3000',
    rating: 4.9,
    image: 'https://images.unsplash.com/photo-1622286342621-4bd786c2447c?w=400',
    description: 'Experiencia premium en cortes y cuidado personal',
    openTime: '09:00',
    closeTime: '22:00',
    latitude: 19.442608,
    longitude: -99.143209
  },
  {
    id: '4',
    name: 'La Navaja de Oro',
    address: 'Calle Juárez #321, Centro Histórico',
    phone: '555-4000',
    rating: 4.7,
    image: 'https://images.unsplash.com/photo-1598523537291-14c52c24070d?w=400',
    description: 'Tradición y calidad en cada corte',
    openTime: '08:00',
    closeTime: '19:00',
    latitude: 19.452608,
    longitude: -99.153209
  }
];

// Barberos simulados
export const mockBarbers: Barber[] = [
  {
    id: '1',
    name: 'Carlos Martínez',
    barbershopId: '1',
    photo: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200',
    specialty: 'Cortes clásicos y barbas',
    rating: 4.9,
    experience: '15 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1621605815971-fbc98d665033?w=300',
      'https://images.unsplash.com/photo-1622286342621-4bd786c2447c?w=300',
      'https://images.unsplash.com/photo-1605497788044-5a32c7078486?w=300'
    ]
  },
  {
    id: '2',
    name: 'Miguel Ángel',
    barbershopId: '1',
    photo: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200',
    specialty: 'Estilos modernos y fade',
    rating: 4.7,
    experience: '8 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?w=300',
      'https://images.unsplash.com/photo-1621607512214-68297480165e?w=300',
      'https://images.unsplash.com/photo-1620331311520-246422fd82f9?w=300'
    ]
  },
  {
    id: '3',
    name: 'Roberto Sánchez',
    barbershopId: '2',
    photo: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=200',
    specialty: 'Diseños y afeitado clásico',
    rating: 4.8,
    experience: '12 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1622287162716-f311baa1a2b8?w=300',
      'https://images.unsplash.com/photo-1621607512214-68297480165e?w=300',
      'https://images.unsplash.com/photo-1605497788044-5a32c7078486?w=300'
    ]
  },
  {
    id: '4',
    name: 'David Hernández',
    barbershopId: '2',
    photo: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=200',
    specialty: 'Cortes urbanos y tendencias',
    rating: 4.6,
    experience: '6 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1620331311520-246422fd82f9?w=300',
      'https://images.unsplash.com/photo-1621605815971-fbc98d665033?w=300',
      'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?w=300'
    ]
  },
  {
    id: '5',
    name: 'Antonio López',
    barbershopId: '3',
    photo: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=200',
    specialty: 'Barbería premium y spa',
    rating: 5.0,
    experience: '20 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1622287162716-f311baa1a2b8?w=300',
      'https://images.unsplash.com/photo-1605497788044-5a32c7078486?w=300',
      'https://images.unsplash.com/photo-1621607512214-68297480165e?w=300'
    ]
  },
  {
    id: '6',
    name: 'Luis García',
    barbershopId: '3',
    photo: 'https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?w=200',
    specialty: 'Cortes ejecutivos',
    rating: 4.8,
    experience: '10 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1620331311520-246422fd82f9?w=300',
      'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?w=300',
      'https://images.unsplash.com/photo-1621605815971-fbc98d665033?w=300'
    ]
  },
  {
    id: '7',
    name: 'Francisco Ruiz',
    barbershopId: '4',
    photo: 'https://images.unsplash.com/photo-1463453091185-61582044d556?w=200',
    specialty: 'Cortes tradicionales',
    rating: 4.7,
    experience: '18 años',
    portfolioImages: [
      'https://images.unsplash.com/photo-1621607512214-68297480165e?w=300',
      'https://images.unsplash.com/photo-1622287162716-f311baa1a2b8?w=300',
      'https://images.unsplash.com/photo-1605497788044-5a32c7078486?w=300'
    ]
  }
];

// Servicios simulados
export const mockServices: Service[] = [
  {
    id: '1',
    name: 'Corte',
    duration: 30,
    price: 150,
    description: 'Corte de cabello profesional'
  },
  {
    id: '2',
    name: 'Premium',
    duration: 60,
    price: 300,
    description: 'Corte de cabello, diseños y barba'
  },
  {
    id: '3',
    name: 'Exfoliación',
    duration: 30,
    price: 120,
    description: 'Masaje facial y toalla caliente'
  }
];

// Citas simuladas (para el barbero) - Con datos históricos para estadísticas
export const mockAppointments: Appointment[] = [
  // Citas futuras y pendientes
  {
    id: '1',
    clientId: '1',
    clientName: 'Juan Pérez',
    clientPhone: '555-0101',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-12-05',
    time: '10:00',
    status: 'confirmed',
    price: 300
  },
  {
    id: '2',
    clientId: '1',
    clientName: 'Juan Pérez',
    clientPhone: '555-0101',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-12-08',
    time: '15:00',
    status: 'pending',
    price: 150
  },
  
  // SEMANA ACTUAL (1-7 Diciembre 2025) - Completadas
  {
    id: '3',
    clientId: '4',
    clientName: 'María González',
    clientPhone: '555-0104',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-12-01',
    time: '10:00',
    status: 'completed',
    price: 300
  },
  {
    id: '4',
    clientId: '5',
    clientName: 'Pedro Ramírez',
    clientPhone: '555-0105',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-12-01',
    time: '14:00',
    status: 'completed',
    price: 300
  },
  {
    id: '5',
    clientId: '6',
    clientName: 'Luis Morales',
    clientPhone: '555-0106',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-12-02',
    time: '11:00',
    status: 'completed',
    price: 150
  },
  {
    id: '6',
    clientId: '7',
    clientName: 'Roberto Silva',
    clientPhone: '555-0107',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-12-02',
    time: '16:00',
    status: 'completed',
    price: 120
  },
  {
    id: '7',
    clientId: '8',
    clientName: 'Diego Torres',
    clientPhone: '555-0108',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-12-03',
    time: '10:30',
    status: 'completed',
    price: 300
  },
  {
    id: '8',
    clientId: '9',
    clientName: 'Carlos Mendoza',
    clientPhone: '555-0109',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-12-03',
    time: '15:00',
    status: 'completed',
    price: 150
  },
  {
    id: '9',
    clientId: '10',
    clientName: 'Fernando Castro',
    clientPhone: '555-0110',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-12-04',
    time: '09:00',
    status: 'completed',
    price: 120
  },
  {
    id: '10',
    clientId: '11',
    clientName: 'Alberto Vega',
    clientPhone: '555-0111',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-12-04',
    time: '12:00',
    status: 'completed',
    price: 300
  },

  // SEMANA ANTERIOR (24-30 Noviembre 2025) - Completadas
  {
    id: '11',
    clientId: '4',
    clientName: 'María González',
    clientPhone: '555-0104',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-11-24',
    time: '10:00',
    status: 'completed',
    price: 150
  },
  {
    id: '12',
    clientId: '5',
    clientName: 'Pedro Ramírez',
    clientPhone: '555-0105',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-11-25',
    time: '11:00',
    status: 'completed',
    price: 300
  },
  {
    id: '13',
    clientId: '6',
    clientName: 'Luis Morales',
    clientPhone: '555-0106',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-11-26',
    time: '14:00',
    status: 'completed',
    price: 120
  },
  {
    id: '14',
    clientId: '7',
    clientName: 'Roberto Silva',
    clientPhone: '555-0107',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-11-27',
    time: '16:00',
    status: 'completed',
    price: 150
  },
  {
    id: '15',
    clientId: '8',
    clientName: 'Diego Torres',
    clientPhone: '555-0108',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-11-28',
    time: '10:00',
    status: 'completed',
    price: 300
  },
  {
    id: '16',
    clientId: '9',
    clientName: 'Carlos Mendoza',
    clientPhone: '555-0109',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-11-29',
    time: '12:00',
    status: 'completed',
    price: 120
  },

  // MES ACTUAL (Noviembre 2025) - Más citas completadas
  {
    id: '17',
    clientId: '10',
    clientName: 'Fernando Castro',
    clientPhone: '555-0110',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-11-15',
    time: '10:00',
    status: 'completed',
    price: 300
  },
  {
    id: '18',
    clientId: '11',
    clientName: 'Alberto Vega',
    clientPhone: '555-0111',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-11-16',
    time: '14:00',
    status: 'completed',
    price: 150
  },
  {
    id: '19',
    clientId: '4',
    clientName: 'María González',
    clientPhone: '555-0104',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-11-18',
    time: '11:00',
    status: 'completed',
    price: 300
  },
  {
    id: '20',
    clientId: '5',
    clientName: 'Pedro Ramírez',
    clientPhone: '555-0105',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-11-20',
    time: '15:00',
    status: 'completed',
    price: 120
  },
  {
    id: '21',
    clientId: '6',
    clientName: 'Luis Morales',
    clientPhone: '555-0106',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-11-22',
    time: '10:30',
    status: 'completed',
    price: 150
  },

  // MES ANTERIOR (Octubre 2025) - Citas completadas
  {
    id: '22',
    clientId: '7',
    clientName: 'Roberto Silva',
    clientPhone: '555-0107',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-10-05',
    time: '11:00',
    status: 'completed',
    price: 300
  },
  {
    id: '23',
    clientId: '8',
    clientName: 'Diego Torres',
    clientPhone: '555-0108',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-10-08',
    time: '14:00',
    status: 'completed',
    price: 150
  },
  {
    id: '24',
    clientId: '9',
    clientName: 'Carlos Mendoza',
    clientPhone: '555-0109',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-10-12',
    time: '10:00',
    status: 'completed',
    price: 300
  },
  {
    id: '25',
    clientId: '10',
    clientName: 'Fernando Castro',
    clientPhone: '555-0110',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-10-15',
    time: '16:00',
    status: 'completed',
    price: 150
  },
  {
    id: '26',
    clientId: '11',
    clientName: 'Alberto Vega',
    clientPhone: '555-0111',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '2',
    serviceName: 'Premium',
    date: '2025-10-18',
    time: '11:00',
    status: 'completed',
    price: 300
  },
  {
    id: '27',
    clientId: '4',
    clientName: 'María González',
    clientPhone: '555-0104',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-10-22',
    time: '09:00',
    status: 'completed',
    price: 120
  },
  {
    id: '28',
    clientId: '5',
    clientName: 'Pedro Ramírez',
    clientPhone: '555-0105',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '3',
    serviceName: 'Exfoliación',
    date: '2025-10-25',
    time: '15:00',
    status: 'completed',
    price: 120
  },
  {
    id: '29',
    clientId: '6',
    clientName: 'Luis Morales',
    clientPhone: '555-0106',
    barberId: '1',
    barberName: 'Carlos Martínez',
    barbershopId: '1',
    barbershopName: 'Barbería El Clásico',
    serviceId: '1',
    serviceName: 'Corte',
    date: '2025-10-28',
    time: '12:00',
    status: 'completed',
    price: 150
  },
];

  // Generar más citas para estadísticas con cantidades de 3 cifras
const generateMoreAppointments = () => {
  const baseAppointments = [...mockAppointments];
  let idCounter = 30;
  
  // Agregar muchas más citas en diciembre (mes actual)
  const services = [
    { id: '1', name: 'Corte', price: 150 },
    { id: '2', name: 'Premium', price: 300 },
    { id: '3', name: 'Exfoliación', price: 120 },
  ];  const clients = [
    { id: '4', name: 'María González', phone: '555-0104' },
    { id: '5', name: 'Pedro Ramírez', phone: '555-0105' },
    { id: '6', name: 'Luis Morales', phone: '555-0106' },
    { id: '7', name: 'Roberto Silva', phone: '555-0107' },
    { id: '8', name: 'Diego Torres', phone: '555-0108' },
    { id: '9', name: 'Carlos Mendoza', phone: '555-0109' },
    { id: '10', name: 'Fernando Castro', phone: '555-0110' },
    { id: '11', name: 'Alberto Vega', phone: '555-0111' },
  ];
  
  // Generar 200 citas para "Corte"
  for (let i = 0; i < 200; i++) {
    const client = clients[i % clients.length];
    const day = (i % 4) + 1; // días 1-4 de diciembre
    baseAppointments.push({
      id: String(idCounter++),
      clientId: client.id,
      clientName: client.name,
      clientPhone: client.phone,
      barberId: '1',
      barberName: 'Carlos Martínez',
      barbershopId: '1',
      barbershopName: 'Barbería El Clásico',
      serviceId: '1',
      serviceName: 'Corte',
      date: `2025-12-0${day}`,
      time: `${9 + (i % 10)}:00`,
      status: 'completed',
      price: 150
    });
  }
  
  // Generar 150 citas para "Premium"
  for (let i = 0; i < 150; i++) {
    const client = clients[i % clients.length];
    const day = (i % 4) + 1;
    baseAppointments.push({
      id: String(idCounter++),
      clientId: client.id,
      clientName: client.name,
      clientPhone: client.phone,
      barberId: '1',
      barberName: 'Carlos Martínez',
      barbershopId: '1',
      barbershopName: 'Barbería El Clásico',
      serviceId: '2',
      serviceName: 'Premium',
      date: `2025-12-0${day}`,
      time: `${9 + (i % 10)}:00`,
      status: 'completed',
      price: 300
    });
  }
  
  // Generar 120 citas para "Exfoliación"
  for (let i = 0; i < 120; i++) {
    const client = clients[i % clients.length];
    const day = (i % 4) + 1;
    baseAppointments.push({
      id: String(idCounter++),
      clientId: client.id,
      clientName: client.name,
      clientPhone: client.phone,
      barberId: '1',
      barberName: 'Carlos Martínez',
      barbershopId: '1',
      barbershopName: 'Barbería El Clásico',
      serviceId: '3',
      serviceName: 'Exfoliación',
      date: `2025-12-0${day}`,
      time: `${9 + (i % 10)}:00`,
      status: 'completed',
      price: 120
    });
  }
  
  return baseAppointments;
};

export const allAppointments = generateMoreAppointments();
